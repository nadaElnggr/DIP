function [image,GMatrix] = Arithmetic(image,GMatrix)
% Create a new figure
fig = figure('Name','Arithmrtic','Color',[1 0.95 0.86],'Position',[350,100,600,400],'MenuBar','none','NumberTitle','off');
% Create the label
label = uicontrol(fig,'style','text','string','Arithmetic','Position',[120,400,400,40],'BackgroundColor',[1 0.95 0.86],'Fontweight','bold','Fontsize',12,'ForegroundColor', [0.23 0.60 0.73],'FontName','Wide Latin');
...
switch(button_number)
    case 1
        string = "AdditionTwoImages";
    case 2
        string = "AdditionOfNumber";
    case 3
        string ="SubtractOfNumber";
    case 4
        string="SubtractTwoImages";
    case 5
        string ="And";
    case 6 
        string ="OR";
...
function buttonCallback(~, ~, button_number)
    switch button_number
    case 1
        SubPlot3(image,GMatrix,"Add two");
        close(fig);
    case 2
        BasicGUI(image,GMatrix,"Add a number");
        close(fig);
    case 3
        BasicGUI(image,GMatrix,"Subtract a number");
        close(fig);
    case 4
        SubPlot3(image,GMatrix,"Subtract 2 images");
        close(fig);
    case 5
        NoInput(image,GMatrix,"AND");
        close(fig);
    case 6
        NoInput(image,GMatrix,"OR");
        close(fig);
    case 7
       Spatial;
       close(fig);
