function Resolution(image, GMatrix)
% Create a new figure
fig = figure('Name','Resolution','Color',[1 0.95 0.86],'Position',[350,100,600,400],'MenuBar','none','NumberTitle','off');
% Create the label
label = uicontrol(fig,'style','text','string','Resolution','Position',[35,400,400,40],'BackgroundColor',[1 0.95 0.86],'Fontweight','bold','Fontsize',12,'ForegroundColor', [0.23 0.60 0.73],'FontName','Wide Latin');
...
function buttonCallback(~, ~, button_number)
    switch button_number
    case 1
        BasicGUI(image,GMatrix,"SubSampling");
        close(fig);
    case 2
        BasicGUI(image,GMatrix,"Resampling");
        close(fig);
    case 3
        ResolutionOptions1(image,GMatrix);
        close(fig);
    case 4
        Spatial;
        close(fig);
